...
but7 = KeyboardButton('Оферта')
but8 = KeyboardButton('Политика конфенциальности')

kb_client = ReplyKeyboardMarkup(resize_keyboard=True)
kb_client.add(but1).add(but2).insert(but3).add(but4).insert(but5).add(but6).add(but7).insert(but8)
...
kb_start = InlineKeyboardMarkup(row_width = 1).add(
    ...
    InlineKeyboardButton("Политика конфиденциальности", callback_data='client_start_conf')
)